...
extern std::vector<Point> vtxData[MAX_SOLID_NUM][MAX_RING_NUM];
Solid* solid = NULL;
...
void makeSolid() {
	...
	for (int solidIdx = 0; solidIdx < solidNum; solidIdx++) {
		...
		for (int i = 1; i != vtxData[solidIdx][0].size(); i++) {
			...
			Euler::mev(pSolid->sface->out_loop, vtx1, vtx2);
			vtx1 = vtx2;
		}
		Euler::mef(pSolid->sface->out_loop, vtx2, startv);
		//做平面中的内环
		...
		for (int i = 1; i <= ringNum[solidIdx]; i++) {
			...
			Euler::mev(pSolid->sface->nextf->out_loop, startv, inLoopStartv);
			vtx1 = inLoopStartv;
			Euler::addVertex(pSolid, vtx1);
			for (int j = 1; j < vtxData[solidIdx][i].size(); j++) {
				...
				Euler::addVertex(pSolid, vtx2);
				Euler::mev(pSolid->sface->nextf->out_loop, vtx1, vtx2);
				vtx1 = vtx2;
			}
			//生成一个面
			Face* face = Euler::mef(pSolid->sface->nextf->out_loop, vtx2, inLoopStartv);
			//删除边
			Euler::kemr(pSolid->sface->nextf->out_loop, startv, inLoopStartv);
			//删除面face
			Euler::kfmrh(pSolid->sface->nextf, face);
		}

		//设定平移扫成方向
		double direc[3] = { 0, 0, 1 };
		//扫成
		Sweep::translation(pSolid, 10, direc);
		//添加一个体
