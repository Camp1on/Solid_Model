...
//获取平面的法向量
void calNormal(Loop* loop, Point& normal) {
	std::vector<Point> cdt;
	HalfEdge* he = loop->he;
	int cnt = 3;
	while (cnt--) {//取三个点
		cdt.push_back(he->ev->cdt);
		he = he->nexth;
	}
	Point a = cdt[1] - cdt[0], b = cdt[2] - cdt[1];
...
void render() {
	...
	while (pSolid != NULL) {
		...
		while (face != NULL) {
			...
			HalfEdge* starte = loop->he, * pedge = NULL, * p = starte;
			while (starte != pedge) {
				Vertex* v = p->ev;
			...
			while (loop != NULL) {
				gluTessBeginContour(tessobj);
				starte = loop->he, pedge = NULL, p = starte;
				while (starte != pedge) {
					Vertex* v = p->ev;
					data[idx][0] = v->cdt.x, data[idx][1] = v->cdt.y, data[idx][2] = v->cdt.z;
					gluTessVertex(tessobj, data[idx], data[idx]);
